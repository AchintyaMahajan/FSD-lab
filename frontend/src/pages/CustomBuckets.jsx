...
import { ChevronLeft, Plus, Trash2, Pin, PinOff, Tag, FolderOpen } from 'lucide-react';
  ...
  const createBucket = async () => {
    ...
    try {
      ...
      if (err?.response?.data?.code === 'UPGRADE_REQUIRED') {
        toast.error('Pro plan required to create custom buckets. Click FREE badge to upgrade!', { duration: 4000 });
      } else {
        ...
        {!isPro ? (
          <div className="glass-card p-12 text-center space-y-4 flex flex-col items-center">
            <div className="w-16 h-16 rounded-2xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center mb-2">
              <FolderOpen size={32} className="text-brand-400" />
            </div>
            <h2 className="text-2xl font-bold text-slate-100">Pro Feature Locked</h2>
            <p className="text-slate-400 max-w-md mx-auto leading-relaxed">
              Unlock Custom Smart Buckets by upgrading to the Pro plan. Automatically route specific senders or subjects into their own organized views.
            </p>
            <button 
          ...
          <>
            <div className="glass-card p-4 sm:p-5 space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <button id="create-bucket-btn" onClick={createBucket} className="btn-primary shrink-0 justify-center"><Plus size={15} /> Create</button>
              </div>
            </div>
                    ...
                    {expanded === b.bucketId && (
                      <div className="border-t border-white/5 p-4 space-y-3 bg-white/[0.02]">
                        ...
                        <div className="flex gap-2 flex-col sm:flex-row pt-1">
                          <select value={newRuleType} onChange={e => setNewRuleType(e.target.value)} className="input w-full sm:w-44 text-xs py-1.5">
                          ...
                          <button onClick={() => addRule(b.bucketId)} className="btn-primary py-1.5 text-xs justify-center"><Plus size={12}/> Add</button>
                        </div>
                      </div>
                    )}
                  </div>
