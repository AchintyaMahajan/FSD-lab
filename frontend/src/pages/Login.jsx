/**
 * pages/Login.jsx — Google OAuth & Email/OTP login page
 */
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Mail, Zap, Shield, Brain, ArrowLeft } from 'lucide-react';
...
const Login = () => {
  ...
  const handleOtpSubmit = async (e) => {
    e.preventDefault();
    if (!otp) return toast.error('OTP code required');

    setIsLoading(true);
    try {
      const { data } = await api.post('/api/auth/verify-otp', { email, code: otp });
      setUser(data.user);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="relative w-full max-w-4xl grid md:grid-cols-2 gap-8 items-center">
        {/* Left — branding (hidden on small mobile, visible on md+) */}
        ...
        <div className="md:hidden text-center space-y-3 fade-in">
          <div className="flex items-center justify-center gap-2.5">
            <div className="p-2 rounded-xl bg-brand-600 shadow-lg shadow-brand-900/50">
              <Mail size={20} className="text-white" />
            </div>
            <span className="text-xl font-bold gradient-text">MasterMail</span>
          </div>
          <p className="text-slate-400 text-sm">AI-powered inbox triage for Gmail</p>
        </div>

        {/* Right — login card */}
        <div className="glass-card p-6 sm:p-8 space-y-6 fade-in relative" style={{ animationDelay: '0.1s' }}>
          
          {view !== 'main' && (
            <button 
              onClick={() => {
                if (view === 'otp') setView('main');
                else setView('main');
              }} 
              className="absolute top-6 left-6 text-slate-400 hover:text-slate-200 transition-colors"
            >
              <ArrowLeft size={18} />
            </button>
          )}
          ...
          <div className="text-center space-y-1 pt-2">
            <h2 className="text-xl font-semibold text-slate-100">
              {view === 'main' ? 'Get started' : 
               view === 'register' ? 'Create an Account' : 
               view === 'login' ? 'Welcome Back' : 'Verification Required'}
            </h2>
            <p className="text-sm text-slate-500">
              {view === 'main' ? 'Sign in to MasterMail' : 
          ...
          {view === 'main' && (
            <div className="space-y-4">
              ...
              <div className="relative py-2 flex items-center justify-center">
                ...
                <div className="relative px-4 text-xs bg-[var(--bg-secondary)] text-slate-500">or sign in with email</div>
              </div>

              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1">Email</label>
                  <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="input text-sm" placeholder="you@example.com" />
                </div>
                <div>
                ...
                <button type="submit" disabled={isLoading} className="btn-primary w-full justify-center py-2.5 mt-2">
                  {isLoading ? 'Sending OTP...' : 'Login & Get Code'}
                </button>
              </form>
              ...
              <div className="text-center pt-2">
                <p className="text-sm text-slate-400">
                  ...
                  <button type="button" onClick={() => setView('register')} className="text-brand-400 hover:text-brand-300 font-medium">
                    Sign Up
                  </button>
                </p>
              </div>
          ...
          {view === 'register' && (
            <form onSubmit={handleRegisterSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={name} onChange={e => setName(e.target.value)} className="input text-sm" placeholder="John Doe" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Email</label>
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="input text-sm" placeholder="you@example.com" />
          ...
          {view === 'otp' && (
            <form onSubmit={handleOtpSubmit} className="space-y-4">
              <div className="text-center mb-4">
                <p className="text-xs text-brand-300">Code sent to <b>{email}</b></p>
              </div>
              <div>
              ...
              <button type="submit" disabled={isLoading || otp.length !== 6} className="btn-primary w-full justify-center py-2.5 mt-2">
          ...
          <div className="border-t border-white/5 pt-4 mt-6 text-center">
            {view === 'main' ? (
              ...
              <p className="text-xs text-slate-500">
                <span className="text-amber-500 font-semibold">Note:</span> MasterMail requires Gmail. Even if you use an email to log in, you will be required to connect a Google account later.
              </p>
            )}
          </div>
        </div>
